...
func InitServer() error {
	once.Do(func() {
		...
		cfg := config.Config{
			...
			TTL:    config.TTLConfig{TTLDefault: 10 * time.Minute},
		}

		log := logger.InitLogger(nil)
		validate := validator.New()

		ch, err := containers.NewValkeyContainer(ctx)
		if err != nil {
			initErr = err
			return
		}
		...
		accessSvc := service.NewAccesses(pg.DB, ch.Cache, log, cfg.TTL, repo)
		if err := accessSvc.ScanAccess(context.Background(), "../../data/route.go"); err != nil {
			initErr = err
			return
		}

		router := router.Api(cfg, pg.DB, ch.Cache, log, validate)
