...
func Api(
	...
	accessService := service.NewAccesses(db, cache, log, cfg.TTL, accessRepository)
	authService := service.NewAuths(log, cfg.Token, userRepository, roleRepository)
	roleService := service.NewRoles(cache, log, cfg.TTL, roleRepository)
	userService := service.NewUsers(db, cache, log, cfg.TTL, userRepository)
